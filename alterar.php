...
<div class="container-xl">
  ...
  if (isset($_SESSION["logado"]))  {
    ...
    <a class="nav-link" href="produtos.php?"> Olá, <?php echo $dadosC['nome']; 
    ?></a>
  <?php  
  }
  if(!isset($_SESSION["logado"])) {?>
  ...
  <?php } 
    ...
    if (isset($_GET["novonome"]) || isset($_GET["novoemail"]) || isset($_GET["novasenha"])  || isset($_GET["novocep"]) || 
    isset($_GET["novocpf"])) {
    ...
    $novocep = $_GET["novocep"];

    $novasenha = $_GET["novasenha"];

    $senha_cripto = password_hash($novasenha, PASSWORD_DEFAULT);

   $_SESSION['senha'] = $_GET["novasenha"];

    $sql = "UPDATE clientes SET nome='$novonome', email='$novoemail', senha='$senha_cripto', 
    cpf='$novocpf', cep='$novocep' WHERE idcliente='$id'";    
    header("location:visualizarconta.php");
    $result = mysqli_query($conexao, $sql);     
    ...
    <tbody>
        <tr>
            <th scope="col">Nome: </th>
             <td><?php  echo "<input type='text' name='novonome' value='$nome'></input>"; ?></td>
        </tr>
        <tr>
            <th scope="row">Email: </th>
            <td><?php echo "<input type='text' name='novoemail' value=$email></input>"; ?></td>
        </tr>
        <tr>
            <th scope="row">Senha: </th>
            <td><?php echo "<input type='text' name='novasenha' value=$senha></input>"; ?></td>
        </tr>    
        <tr>
            <th scope="row">CPF: </th>
            <td><?php echo "<input type='text' name='novocpf' value=$cpf></input>"; ?></td>
        </tr>
        <tr>
            <th scope="row">CEP: </th>
            <td><?php echo "<input type='text' name='novocep' value=$cep></input>"; ?></td>
        </tr>  
  </tbody>
</table>
...
<div class="container">
    <div class="row justify-content-md-center mb-4">
        <div class="col-md-auto mb-2">
            <a href="visualizarconta.php"><input type="submit" value="Salvar" name="salvar" class="btn btn-danger"></a>
        </div>
        <div class="col-md-auto">
            <a href="visualizarconta.php"><button type="button" class="btn btn-danger">Voltar</button></a>
        </div>
    </div>
</div>
</form>
